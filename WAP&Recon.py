	...
	def main(self,argv):
		...
		try:
			opts,arg = getopt.getopt(
				argv,'u:s:',['url=','scan=','wappalyzer','crawler','agent=','random-agent','redirect=',
		...
		for o,a in opts:
			...
			if o in ('-s','--scan'):
				self.scan = a
			if o in ('--wappalyzer'):
			 	print("\nwappalyzer: "+str(wappalyzer.analyze(WebPage.new_from_url(self.url)))+"\n")	    
		...
		if self.scan == '0' or self.scan == '3':
			print self.nw
			urls = self.ma.crawling(agent,proxy,redir,time,self.url,cookie)
			if urls==[]:urls.append(self.url)
		if self.scan == '0':
			self.ma.bruteforce(agent,proxy,redir,time,self.url,cookie)
			self.ma.disc(agent,proxy,redir,time,self.url,cookie)
			self.ma.attacks(agent,proxy,redir,time,urls,cookie)
			self.ma.others(agent,proxy,redir,time,self.url,cookie)
			self.ma.vuln(agent,proxy,redir,time,self.url,cookie)
		if self.scan == '1':
			self.ma.bruteforce(
